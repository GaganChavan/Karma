// ─── KARMA APP — HABITS SCREEN (PHASE F-2) ───────────────────────────
// Phase F-2: category tag shown on each habit row
// Everything else identical to Phase E (pause/resume, 7-day dots, etc.)
...
// Phase F-2: Category config for tags
const CATEGORY_TAG = {
  spiritual:    { icon: '🕉️', label: 'Spiritual',    color: '#BF5AF2' },
  intellectual: { icon: '📚', label: 'Intellectual', color: '#0A84FF' },
  physical:     { icon: '💪', label: 'Physical',     color: '#30D158' },
...
const HabitsScreen = ({ navigation }) => {
  ...
  const _renderHabit = (habit, isArchived = false) => {
    ...
    // Phase F-2: category tag
    const catTag = CATEGORY_TAG[habit.category];
    ...
    return (
      ...
      >
        ...
        <View style={{ flex: 1, gap: 3 }}>
          ...
          {/* Type + frequency + Phase F-2: category tag */}
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
            ...
            {/* Phase F-2: category pill */}
            {catTag && (
              <>
                <Text style={{ ...Typography.caption1, color: colors.textDim }}>·</Text>
                <View style={{
                  flexDirection: 'row', alignItems: 'center', gap: 2,
                  backgroundColor: catTag.color + '18',
                  borderRadius: Radius.full,
                  paddingHorizontal: 6, paddingVertical: 1,
                }}>
                  <Text style={{ fontSize: 9 }}>{catTag.icon}</Text>
                  <Text style={{ fontSize: 9, color: catTag.color, fontWeight: '600' }}>{catTag.label}</Text>
                </View>
              </>
            )}
          </View>

          {/* 7-day dots — active habits only */}
          {!isArchived && (
            <WeekDots
              habitId={habit.id}
              weekDates={weekDates}
            ...
            <View style={{ backgroundColor: colors.backgroundCard, borderRadius: Radius.lg, overflow: 'hidden', borderWidth: 1, borderColor: colors.separator }}>
              {paused.map((habit, i) => {
                ...
                return (
                  <View key={habit.id}>
                    {i > 0 && <View style={{ height: 1, backgroundColor: colors.separator, marginHorizontal: Spacing.lg }} />}
                    <View style={{ flexDirection: 'row', alignItems: 'center', padding: Spacing.lg, gap: Spacing.md, opacity: 0.65, borderLeftWidth: 3, borderLeftColor: colors.textMuted }}>
                      ...
                      <View style={{ flex: 1, gap: 3 }}>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                          <Text style={{ ...Typography.caption1, color: colors.textMuted }}>
                            Paused · Logs preserved
                          </Text>
                          {catTag && (
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2, backgroundColor: catTag.color + '18', borderRadius: Radius.full, paddingHorizontal: 6, paddingVertical: 1 }}>
                              <Text style={{ fontSize: 9 }}>{catTag.icon}</Text>
                              <Text style={{ fontSize: 9, color: catTag.color, fontWeight: '600' }}>{catTag.label}</Text>
                            </View>
                          )}
