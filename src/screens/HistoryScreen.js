// ─── KARMA APP — HISTORY SCREEN (PHASE 5) ────────────────────────────
// Browse past 30 days. See all habits status per day.
// Edit past check-ins up to 3 days back.
...
const HistoryScreen = ({ navigation }) => {
  ...
  const _loadData = async () => {
    try {
      ...
      // Get all checkins for last 30 days
  ...
  const _handleEditCheckin = async (habit, date, currentStatus) => {
    ...
    if (!canEdit) {
      Alert.alert(
        '🔒 Can\'t Edit',
        'You can only edit check-ins up to 3 days back. This keeps your data honest, Neel.',
        [{ text: 'OK' }]
      );
      return;
    }
    ...
    Alert.alert(
      `Edit: ${habit.name}`,
      `${DateUtils.formatDate(date)}\nCurrent: ${STATUS_CONFIG[currentStatus]?.label || 'No data'}`,
      [
        ...options.map(o => ({
          text:    o.text,
          onPress: async () => {
    ...
    <SafeAreaView style={styles.screen} edges={['top']}>
      ...
      >
        <Text style={styles.editHint}>
          Tap any habit status to edit (up to 3 days back)
        </Text>
          ...
          <View key={day.date} style={styles.dayCard}>
            ...
            <View style={styles.dayProgressTrack}>
              <View style={[styles.dayProgressFill, {
                ...
                backgroundColor:
                  ...
                  day.doneCount > 0 ? Colors.blue : Colors.backgroundCard,
              }]} />
            </View>
            ...
            {day.habitStatuses.map(({ habit, status }) => {
              ...
              return (
                <TouchableOpacity
                  key={habit.id}
                  style={[styles.habitRow, canEdit && { opacity: 1 }]}
