    ...
    if (nnIds.length > 0) {
      const nnDone = await db.getFirstAsync(
        `SELECT COUNT(*) as count FROM checkins
         WHERE habit_id IN (${nnIds.join(',')}) AND date >= ? AND status IN ('done','resisted')`,
        [startDate]
      );
...
export const getBeforeNowData = async (alterEgo = 'Neel') => {
  try {
    ...
    const earlyResult = await db.getFirstAsync(
      `SELECT COUNT(*) as total, SUM(CASE WHEN status IN ('done','resisted') THEN 1 ELSE 0 END) as done
       FROM checkins WHERE date >= ? AND date < ?`,
      [fmt(ninetyDaysAgo), fmt(sixtyDaysAgo)]
    );
    const earlyRate = earlyResult?.total > 0
      ? Math.round((earlyResult.done / earlyResult.total) * 100) : 0;
    ...
    const recentRate = recentResult?.total > 0
      ? Math.round((recentResult.done / recentResult.total) * 100) : 0;
    ...
    return {
      earlyRate, recentRate, improvement,
